...
new Command({
    ...
    ctx.reply(`§cFroze §f"§a${player.name}§f" Because: "${reason ?? "No reason Provided"}" §aSuccessfully`);
    ctx.sender.tell(`§cYou have been frozen by §f"§a${ctx.sender.nameTag}§f" Because: "${reason ?? "No reason Provided"}"`);
});
