...
new Command({
    ...
    ctx.reply(`§cMuted §f"§a${player.name}§f" §cfor ${length} ${unit} Because: "${reason ?? "No reason Provided"}" §aSuccessfully`);
    player.tell(`§cYou have been muted by §f"${ctx.sender.name}" §cfor ${length} ${unit} Because: "${reason ?? "No reason Provided"}"`);
});
