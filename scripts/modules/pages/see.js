import { MinecraftItemTypes, world, } from "mojang-minecraft";
...
export function ViewPlayerEnderChestFill(entity, page, extras) {
    ...
    for (const item of Object.values(MinecraftItemTypes)) {
        try {
            player.runCommand(`testfor @s[hasitem={item=${item.id},location=slot.enderchest}]`);
...
new Page("moderation:see", 54, ViewPlayersFill)
    .setSlots([45, 46, 47, 49, 51, 52, 53], new Item("minecraft:stained_glass_pane", 1, 15, {
    nameTag: "§r",
}), (ctx) => {
    ctx.SetAction();
})
    .setSlots([50], new Item("minecraft:arrow", 1, 0, {
    nameTag: "§fBack",
}), (ctx) => {
    ctx.PageAction("home");
})
