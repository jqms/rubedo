...
export function getRole(player: Player | string): keyof typeof ROLES {
  if (typeof player == "string") {
    // if player is in the game grab there role
    // if not grab the offline data in database
    const inGamePlayer = [...world.getPlayers()].find((p) => p.name == player);
    if (inGamePlayer) {
      return inGamePlayer.getDynamicProperty("role") as keyof typeof ROLES;
    } else {
      return TABLES.roles.get(player) ?? "member";
    }
  } else {
    // grab straight from player
    return player.getDynamicProperty("role") as keyof typeof ROLES;
  }
}
...
export function getId(entity: Entity): string {
  try {
    // Check to see if this entity has a score already set
    return entity.scoreboard.id.toString();
  } catch (error) {
    // entity does not have a set score so we need to give them a score
    try {
      try {
      ...
      // entity couldnt use scoreboard or something broke
      const tagId = entity.getTags().find((t) => t.startsWith("id:"));
      if (tagId) return tagId.replace("id:", "");
      const id = Date.now();
      entity.addTag(`id:${id}`);
      return id.toString();
    }
